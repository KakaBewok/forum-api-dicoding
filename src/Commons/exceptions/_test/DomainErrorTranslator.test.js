...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    expect(
      DomainErrorTranslator.translate(
        new Error('REGISTERED_USER.NOT_CONTAIN_NEEDED_PROPERTY')
      )
    ).toStrictEqual(
      new InvariantError(
        'tidak dapat membuat user baru karena properti yang dibutuhkan tidak ada'
    ...
    expect(
      DomainErrorTranslator.translate(
        new Error('REGISTERED_USER.NOT_MEET_DATA_TYPE_SPECIFICATION')
      )
    ).toStrictEqual(
      new InvariantError(
        'tidak dapat membuat user baru karena tipe data tidak sesuai'
    ...
    expect(
      DomainErrorTranslator.translate(
        new Error('NEW_AUTH.NOT_CONTAIN_NEEDED_PROPERTY')
      )
    ).toStrictEqual(new InvariantError('harus mengirimkan token'));
    expect(
      DomainErrorTranslator.translate(
        new Error('NEW_AUTH.NOT_MEET_DATA_TYPE_SPECIFICATION')
      )
    ).toStrictEqual(new InvariantError('token harus string'));
    // thread
    expect(
      DomainErrorTranslator.translate(
    ...
    expect(
      DomainErrorTranslator.translate(new Error('ADD_THREAD.NO_AUTHORIZATION'))
    ...
    // comment
    expect(
      DomainErrorTranslator.translate(new Error('ADD_COMMENT.NO_AUTHORIZATION'))
    ).toStrictEqual(new AuthenticationError('Missing authentication'));
    expect(
      DomainErrorTranslator.translate(new Error('ADD_COMMENT.NO_PARAMS'))
    ).toStrictEqual(new NotFoundError('params tidak ditemukan'));
    expect(
      DomainErrorTranslator.translate(
    ...
    expect(
      DomainErrorTranslator.translate(
        new Error('ADD_COMMENT.CONTENT_LIMIT_CHAR')
      )
    ).toStrictEqual(
      new InvariantError(
    ...
    expect(
      DomainErrorTranslator.translate(
        new Error('DELETE_COMMENT.NO_AUTHORIZATION')
      )
    ).toStrictEqual(new AuthenticationError('Missing authentication'));
    expect(
      DomainErrorTranslator.translate(new Error('DELETE_COMMENT.NO_PARAMS'))
    ).toStrictEqual(new NotFoundError('parameter komen id tidak ada'));
    expect(
      DomainErrorTranslator.translate(
...
// expect(
//   DomainErrorTranslator.translate(new Error('ADD_COMMENT.NOT_FOUND_THREAD'))
// ).toStrictEqual(new InvariantError('Thread tidak ditemukan'));
